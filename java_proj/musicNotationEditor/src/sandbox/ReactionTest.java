...
public class ReactionTest extends Window {
    ...
    public ReactionTest() {
        ...
        Reaction.initialReactions.addReaction(new Reaction("SW-SW") {
            public int bid(Gesture g) {return 0;} // a perfect bid
    ...
    public static class Box extends Mass {
        ...
        public Box(G.VS vs) {
            super("BACK");
            this.vs = vs; // forget this line, causing error

            addReaction(new Reaction("S-S") { // this reaction: delete the box
                public int bid(Gesture g) {
                    int x = g.vs.xM(), y = g.vs.yL();
                    if (Box.this.vs.hit(x, y)) {
                        return Math.abs(x - Box.this.vs.xM());
                    } //Box.this syntax: tie to the Box it created
            ...
            addReaction(new Reaction("DOT") { // this reaction: change color
                public int bid(Gesture g) {
                    int x = g.vs.xM(), y = g.vs.yL();
                    // if x, y are closed, change color
                    // if the squares are overlapping, see (x, y) is closer to which box, change the color of that box
                    if (Box.this.vs.hit(x, y)) {
